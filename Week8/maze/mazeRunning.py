...
class state():
    # It stores the row and column coordinates (i and j), the number of steps taken to reach this state (g), and a reference to the parent state (parent).
...
def valid(i, j):
    # The valid function checks if a given cell at (i, j) is a valid position to move to:
    return 0 <= i < rows and 0 <= j < cols and maze[i][j] == '0'


def successor(s):
    # The successor function generates valid successor states for a given state s.
    succ = []
    for y, x in adj:
